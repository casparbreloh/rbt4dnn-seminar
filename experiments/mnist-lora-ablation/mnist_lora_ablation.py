...
FIELDS = [
    ...
    "estimated_valid_failures_per_1000",
...
def build_rows(rows: list[CsvRow]) -> list[CsvRow]:
    ...
    for row in sorted(mnist_rows, key=lambda item: (item["requirement"], item["method"])):
        estimated_valid_failure_rate = metric(row)
        ...
        out.append(
            {
                ...
                "estimated_valid_failures_per_1000": f"{1000 * estimated_valid_failure_rate:.3f}",
                "relative_to_per_requirement_lora": f"{estimated_valid_failure_rate / base:.3f}",
            }
        )
    return out
...
def summary(rows: list[CsvRow]) -> str:
    ...
    lines = [
        ...
        "",
        "**Question:** Does MNIST really need one LoRA per requirement, or can the paper's "
        "shared ablations produce useful valid failures too?",
        "",
        "**Method:** Compare per-requirement LoRA (`lr`) with the paper's shared MNIST "
        "`allreq` and `alldata` ablations using estimated valid failures per 1000 images.",
        "",
        "**Result:** Shared `allreq` is best for M1, M4, M5, and M6; per-requirement LoRA "
        "is best for M2 and M3.",
        "",
        "**Limitation:** This reuses the paper's aggregate pass and precondition rates; it "
        "does not train new LoRAs.",
        "",
        "## Details",
        "",
    ]
    for requirement, req_rows in sorted(by_requirement.items()):
        best = max(
            req_rows,
            key=lambda item: float(item["estimated_valid_failures_per_1000"]),
        )
        lr = next(row for row in req_rows if row["method"] == "lr")
        lines.append(
            f"- {requirement}: best estimated valid-failure efficiency is `{best['method']}` "
            f"({best['estimated_valid_failures_per_1000']} per 1000). "
            "`allreq/alldata` ratios are measured against "
            f"`lr={lr['estimated_valid_failures_per_1000']}`."
        )
    return "\n".join(lines) + "\n"
